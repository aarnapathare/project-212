...
def stop():
    ...
    mixer.music.load("shared_files/"+song_selected)
    mixer.music.pause()
    mixer.music.configure(text="")
    stop = Button(window, text="Stop", width =10, bd=1, bg="skyBlue", font=("Calibri", 10), command="play")
    stop.place(x=10, y=200)
...
def resume():
    ...
    resumeButton = Button(window, text="Resume", width =10, bd=1, bg="skyBlue", font=("Calibri", 10), command="play")
    resumeButton.place(x=10, y=200)
...
def pause():
    ...
    pauseButton = Button(window, text="Pause", width =10, bd=1, bg="skyBlue", font=("Calibri", 10), command="play")
    pauseButton.place(x=10, y=200)
def browseFiles():
    global Listbox
    global song_counter
    ...
    try:
        ...
        Listbox.inster(song_counter, fname)
...
def download():
    ...
    infolable.configure(text="downloading" + song_to_download)
    HOSTNAME = "127.0.0.1"
    USERNAME = "lftpd"
    PASSWORD = "lftpd"
    home = str(Path.home())
